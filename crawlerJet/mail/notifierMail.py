...
def sendNotifierMail(date, person, cityPair, airplaneList):
    ...
    message = MIMEText(NOTIFIER_MSG.format(airplaneCurrency, person['money'], cityPair[0], cityPair[1], airplanesStr))
    message['from'] = PARAM.EMAIL_FROM_ADDR
    message['to'] = person['email']
    message['subject'] = NOTIFIER_SUBJECT.format(cityPair[0], cityPair[1], strDate)

    body = {'raw': base64.urlsafe_b64encode(message.as_string())}
    try:
        gmail.users().messages().send(userId='me', body=body).execute()
        logging.warning('Success: send cheaper email ({0} -> {1})'.format(cityPair[0], cityPair[1]))
    except Exception as e:
